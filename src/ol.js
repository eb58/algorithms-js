const ol = (() => {
  ...
  const counter = (to, step = 1) => {
    return {
      foreach: (f) => {
        for (let i = 1; i <= to; i += step) f(i)
      },
      reduce: (f, acc) => {
        for (let i = 1; i <= to; i += step) acc = f(acc, i)
        return acc
      }
    }
  }

  const uid = (prefix = '') => prefix + randomInRange(1_000_000, 9_999_999) + performance.now() // nobody needs uuid() !!!
