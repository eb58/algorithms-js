...
const ol = {
  ...
  sqr: (x) => x ** 2,
  cube: (x) => x ** 3,
  fac: (x) => ol.range(x).map(ol.inc).reduce((acc, n) => acc * n, 1),
  ...
  max: (xs, proj) => proj
    ? xs.reduce((a, x) => proj(x) > proj(a) ? x : a, xs[0])
    : xs.reduce((a, x) => x > a ? x : a, xs[0]),
  min: (xs, proj) => proj
    ? xs.reduce((a, x) => proj(x) < proj(a) ? x : a, xs[0])
    : xs.reduce((a, x) => x < a ? x : a, xs[0]),
...
tryAction = (action, finalAction) => {
  try { action() }
  catch (e) { console.log("tryAction", e); throw "tryAction" + e }
...
logtor = (f) => {
  ...
  const blanks = range(100).map(() => ' ').join('');
