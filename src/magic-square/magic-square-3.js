...
const magicSquare = (n) => {
  ...
  const rows = [
    ...
    {
      row: [1, 2],
      restriction: (square, availableNumbers, perm) => {
        if (square[0] + perm[0] + perm[1] + square[3] !== MN) return false

        if (!availableNumbers.includes(MN - (perm[0] + square[5] + square[9]))) return false
        if (!availableNumbers.includes(MN - (perm[1] + square[6] + square[10]))) return false
    ...
    },
    {
      ...
      restriction: (square, availableNumbers) => {
        if (!availableNumbers.includes(MN - (square[1] + square[5] + square[9]))) return false
        return true
      },
      setValue: (square) => {
    ...
    },
    {
      row: [14],
      restriction: (square, availableNumbers) => {
        if (!availableNumbers.includes(MN - (square[2] + square[6] + square[10]))) return false
        return true
      },
      setValue: (square) => {
    ...
    },
    {
      row: [4, 8],
      restriction: (square, availableNumbers) => {
        if (square[0] + perm[0] + perm[1] + square[12] !== MN) return false

        if (!availableNumbers.includes(MN - (perm[0] + square[5] + square[6]))) return false
        if (!availableNumbers.includes(MN - (perm[1] + square[9] + square[10]))) return false
  ...
  const combineToMagicSquare = (square, combis, availableNumbers, lev, res) => {
    ...
    combis.forEach((combi, idx) => {
      combi.perms.forEach((perm) => {
        if (row.restriction && !row.restriction(square, availableNumbers, perm)) return

        if (row.setValue) {
          const lsquare = [...square]
          const valueSet = row.setValue(lsquare)
          const newAvailableNumbers = availableNumbers.filter((n) => n !== valueSet)
          newCombis = combinations(newAvailableNumbers, rows[lev + 1].row.length).map((xs) => ({
            numbersArray: xs,
            perms: perm(xs),
          }))
