...
permX = xs => { // Heaps algorithm --- https://en.wikipedia.org/wiki/Heap%27s_algorithm
  const len = xs.length;
...
perm4 = xs => xs.length === 4 ? perms4(xs) : xs.length === 3 ? perms3(xs) : xs.length === 2 ? perms2(xs) : xs.length === 1 ? perms1(xs) : permX(xs)
