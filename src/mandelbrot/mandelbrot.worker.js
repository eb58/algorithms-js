...
const mandelbrot = (cx, cy, maxIterations = 100, COLORS) => {
  let [x, y, n] = [cx, cy, 0]
  if ((x + 0.25) * (x + 0.25) + y * y < 0.25) return getCol(n, 0, COLORS)
  if ((x + 1) * (x + 1) + y * y < 0.06) return getCol(n, 0, COLORS)
  while (n++ < maxIterations && x * x + y * y <= 4) {
    const xTemp = x * x - y * y + cx
    y = 2 * x * y + cy
    x = xTemp
  }
  return getCol(n, maxIterations, COLORS)
}
...
onmessage = (msg) => {
  ...
  for (let r = data.startRow, rr = 0; r < data.endRow; r++, rr++) {
    ...
    for (let c = 0; c < width; c++) {
      ...
      chunkImageData.set(mandelbrot(x, y, view.maxIterations, data.COLORS), (rr * width + c) * 4)
    }
  }
